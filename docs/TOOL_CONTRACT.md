...
| `message_send_file` | true | Send one workspace file to the current bot chat as a structured attachment. Rejects paths outside the workspace root. |
| `message_send_image` | true | Send one workspace image to the current bot chat as an image attachment. Rejects non-image files and paths outside the workspace root. |
...
| `schedule_cancel` | false | Cancel (delete) a scheduled task for the current chat. Use schedule_list first when you need the task id. |
| `schedule_delete` | false | Delete a scheduled task created for the current chat. Use schedule_list first when you need the task id. |
| `schedule_list` | true | List scheduled tasks for the current chat so the assistant can inspect, confirm, or choose one to modify. |
| `schedule_pause` | false | Pause a scheduled task in the current chat. The task can be resumed later with schedule_resume. |
| `schedule_resume` | false | Resume a paused scheduled task in the current chat. |
| `schedule_task` | false | Create a recurring scheduled task for the current chat. The task will automatically run the given prompt or message on the requested schedule. Schedules must be at least one minute, for example `1h`, `30m`, `daily@09:00`, or `weekly:mon@09:00`. |
