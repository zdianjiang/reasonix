...
func TestCreateTaskCreatesHostOwnedLayoutAndInitialState(t *testing.T) {
	...
	wantRoot := filepath.Join(root, ".agents", "autoresearch", task.ID)
...
func TestLoadTaskRejectsSymlinkTaskDirectory(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".agents", "autoresearch"), 0o755); err != nil {
		t.Fatalf("create autoresearch root: %v", err)
	}
	if err := os.Symlink(outside, filepath.Join(root, ".agents", "autoresearch", taskID)); err != nil {
