...
// canonical .agents project dir is highest priority (last, since command.Load
...
func TestCommandDirsIncludeConventions(t *testing.T) {
	...
	// The project's .agents/commands must be the highest-priority (last) entry.
	if last := dirs[len(dirs)-1]; last != filepath.Join(".agents", "commands") {
		t.Errorf("project .agents/commands should be highest priority (last), got %q", last)
	}
}
