...
// .agents/attachments and returns its repo-relative path for @referencing.
...
func cleanAttachmentPath(path string) (string, error) {
	...
	root := filepath.Join(config.ProjectDirname, "attachments")
	if clean == "." || clean == root || strings.HasPrefix(clean, ".."+string(filepath.Separator)) || !strings.HasPrefix(clean, root+string(filepath.Separator)) {
		return "", fmt.Errorf("attachment path is outside .agents/attachments")
	}
	if err := ensureAttachmentRoot(); err != nil {
		return "", err
	}
...
func ensureAttachmentRootIn(base string) error {
	root := filepath.Join(base, config.ProjectDirname, "attachments")
...
func attachmentPath(ext string) string {
	...
	return filepath.Join(config.ProjectDirname, "attachments", name)
}
